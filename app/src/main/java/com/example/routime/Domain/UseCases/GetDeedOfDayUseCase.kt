...
class GetDeedOfDayUseCase(context: Context) {
    private val deedRepo : DeedRepo = DeedRepoImp(context)

    operator fun invoke(dateTimeInMillis: Long ):Flow<List<Deed>>{
        val dateFormattedForDb = Utils.formatTimeForDataBase(Date(dateTimeInMillis),"yyyy-MM-dd")+"%"
        return deedRepo.getDeedOfDay(dateFormattedForDb).flowOn(Dispatchers.IO)
    }
}